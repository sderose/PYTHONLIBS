...
* '''months''' Month names and abbreviations (3-letter plus "Sept").

* '''weekdays''' Weekday names and abbreviations (3-letter plus "Thur" and "Thurs").


* '''relativeDays''' "today|tomorrow|yesterday|eve"

* '''dayParts''' terms for parts of days, such as "noon", "lauds", etc.

* '''titles''' Personal titles, including some religious, military,
and governmental; and their abbreviations.

* '''unitPrefixes''' Metric prefixes, such as "kilo".

==More complex lists include==

* '''countryCodes''' A Python `dict` keyed by 2-letter country code.
The values are 3-tuples of the corresponding 3-letter code, numeric code, and
full name (in ASCII only). For example:
    'FO': ('FRO', 234, "Faroe Islands"),
    'FJ': ('FJI', 242, "Fiji"),

* '''contractionList''' A Python `dict` mapping contracted forms to
...
This does not include productive cases such as `[noun]'s` for possessive
...
* '''semiRegularContractionList''' Is a Python dict mapping (fairly)
productive contractions to their expansions and a list of the word classes
they apply to. For example:
    "n't"                 : "not",        [ "MD" ] ],
    "'n"                [ "than",       [ "JJR" ] ],

Cases that are already covered by the explicit ''contractionList''
(such as "I'd"), do not trigger adding their POS to the ''semiRegularContractionList''.
...
And finally, one method:
    ...
    doContractions(self, s) -- This will take the string ''s'', and replace any
contractions found in ''contractionList'', with their expanded forms. It then
applies the ''semiRegularContractionList'', ''but'' it does not check the POS
...
Copyright 2012-08-22 by Steven J. DeRose. This work is licensed under a Creative Commons
