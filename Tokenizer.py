...
=item * The I<SimpleTokenizer> class is quick and should be adequate for
...
=item B<normalize> -- What Unicode normalization to apply.
Default: 'NKFD'.

=item B<breakHyphens> -- Whether to split hyphenated words. Default: False.

=item B<fancyContractions> -- Whether to use the I<TokensEN> package
to handle contractions, or just some local regexes. Default: False.

=item B<verbose> -- Whether to print steps of progress. Default: False.

=back


=item * I<NLTKTokenizerPlus> applies the NLTK tokenizer, then
makes a few adjustments. Usage is the same as just shown,
just substitute "NLTKTokenizerPlus".

=item * I<HeavyTokenizer> provides a variety of useful but complex steps, and
...
=item * Many kinds of non-word tokens, such as URIs, Twitter hashtags, userids,
...
=item * Contemporary conventions such as emphasis via special puntuations
(*word*), or via repeating letters (aaaarrrrrggggghhhhhh, hahahaha).

=item * Special cases such as contractions and possessives
...
=head1 Some high-order problem cases (not including Unicode cases)
...
=head1 Differences from some other tokenizers
